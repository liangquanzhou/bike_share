...
So we cound conlude that people are likely use bike as a transportation to their office in weekdays, and just riding for fun during the weekend.

### Trip duration 

Then immediately, one may want to find out if the durations of usage in weekdays and weekends will differ:

```{r}
gridExtra::grid.arrange(p5, p6, ncol = 1)
```

The result also confirmed out guess: people are buzy in weekdays and mainly use bike to commute, but they have more time enjoy riding in weekends. 

### Locations

Bike stations are avaliable in 4 cities: San Francisco, San Jose, Mountain View, and Palo Alto. Here are the locations:
...
```
More specificly, we want to conduct a multiple linear regression between the number of trips in a day and weather-realted variables. In this model we only use San Francisco data, which has the most usage among 4 cities.
...
We seleted 8 initlal predictor variables:
...
We can find that many of these days are national holidays or days around national holidays. So seperating days by weekday/weekend may not accurate.

Treat national holidays also as weekend, and do $Y = \sqrt{Y}$ transformation on response varible, and using the same variable selection, then we can get the final model: 
